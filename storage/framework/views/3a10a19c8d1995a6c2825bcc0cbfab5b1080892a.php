...
<body>
	...
	<div class="alert alert-danger">
		<div class="Huialert Huialert-danger">
			<button type="button" class="close  btn-info" data-dismiss="alert" aria-label="Close" style="width:100%;color:black;opacity:1;font-family: 宋体;font-weight: bold" style="margin-bottom:1px;position: relative;"><i class="icon Hui-iconfont" style="color:black;font-weight:bold;position:absolute;right:60%;top:31%;font-size:20px;color:red;opacity:1.2">&#xe691;</i><?php echo e(session('error')); ?>
	...
	<div class="alert alert-danger">
		<div class="Huialert Huialert-error">
			<button type="button" class="close  btn-danger" data-dismiss="alert" aria-label="Close" style="width:100%;color:black;opacity:1;font-family: 宋体;font-weight: bold" style="margin-bottom:1px;position: relative;"><i class="icon Hui-iconfont" style="color:black;font-weight:bold;position:absolute;right:60%;top:31%;font-size:<?php echo e(20*count($errors)); ?>px;color:red;opacity:1.2">&#xe691;</i>
	...
	<?php endif; ?>
	<?php if(session('controller')[1]=='create'||session('controller')[1]=='edit' ||session('controller')[1]=='show' ||session('controller')[1]=='picadd'
	||session('controller')[1]=='attrshow'): ?>
	<?php else: ?>
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span><?php echo e(session('controller')[0]); ?>管理 <span class="c-gray en">&gt;</span> <?php echo e(session('controller')[0]); ?>列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	...
	<a class="btn btn-primary radius" onclick="picture_add('添加<?php echo e(session('controller')[0]); ?>','/admin<?php echo e($mname); ?>/create<?php echo e('?sid='.$_GET['sid']); ?>')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加该商品库存</a>
	<?php else: ?>
	<a class="btn btn-primary radius" onclick="picture_add('添加<?php echo e(session('controller')[0]); ?>','/admin<?php echo e($mname); ?>/create')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加<?php echo e(session('controller')[0]); ?></a>
	<?php endif; ?>
	</span> <span class="r">共有数据：<strong><?php echo e(count($data)); ?></strong> 条</span> </div>
	<?php endif; ?>
	<input type="hidden" value="<?php echo e(session('success')); ?>" id="test111">
...
function datadel()
{
	...
	$('.null>input:checked').each(function(){
		...
		$.ajax({
			type: 'GET',
			url: '/admin<?php echo e($mname); ?>delall',
			data: {id:id},
			dataType: 'json',
			success: function(data){
				if(data.result==1){
...
function link_display(obj,id,display){
	// var status=new Array("1","0");
			$.ajax({
			type: 'GET',
			url: '/admin<?php echo e($mname); ?>status',
...
<?php if(session('success')): ?>
	<script>	
		...
		setTimeout(function()
		{    
			parent.layer.msg("<?php echo e(session('success')); ?>",{icon:1,time:1500});
		},50);
	</script>
