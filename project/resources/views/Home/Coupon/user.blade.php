...
<html>
	...
	<head>
		...
		<link rel="stylesheet" href="/static/homecss/sui.css" />
				...
				<div class="cont-main">
					<div class="main-wrap mt15" style="border: 0px;">
						<h3 style="background-color:#fff ;padding: 15px;width: 980px;">
	                        <strong style="font-size: 18px;color: #333333;">我的购物券</strong>
						</h3>
								 ...
								 <div style="margin-top: 20px;">
								 	...
								 	<div style="float: left; ">
								 		<div style="background-image: url(/static/home/images/z-44.png);width: 226px;height: 288px;">
								 			@if ( date('Y-m-d h:i:s',time()) > $value->cstart && date('Y-m-d h:i:s',time()) < $value->cstop )
											...
											@endif
											@if ( date('Y-m-d h:i:s',time()) < $value->cstart)
											...
											@endif
								 		    ...
								 		    <span style="font-size: 40px;color: #fff;margin-top: 50px;display: block;position: absolute;margin-left: 65px;">￥{{$value->cpri}}</span>
								 		    
								 		    <dl style="color: #fff;font-size: 12px;position: absolute;margin-top:100px;margin-left: 65px;">[满{{$value->cmin}}元减{{$value->cpri}}元]</dl>
								 		     <dl style="color: #fff;font-size: 12px;position: absolute;margin-top:120px;margin-left: 40px;">{{$value->cstart}}至{{$value->cstop }}</dl>
								 		     
								 		    <dl style="color: #686868;font-size: 12px;position: absolute;margin-top:170px;margin-left: 20px;">券号：{{$value->cid}}
								 		    <dl style="color: #686868;font-size: 12px;position: absolute;;">适用商品：{{$cate[$key]->name}}
								 		    	
								 		   <a   href="/">
								 		    <span style="float:left;border: 1px #81cccd solid;color: #81cccd;padding: 2px;display: block;text-align: center;margin-left: 100px;margin-top: -20px;    width: 80px;" >去使用  >  </span></a>
