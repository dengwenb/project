	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		@foreach($data as $row)
			<tr class="text-c">
				...
				<td >{{$row->email}}</td>
				<td >{{$row->rolename}}</td>
				<td class="sta">{{$row->status}} </td>
				<td class="que"> {{$row->questatus}}</tds>
				<td >@if($row->questatus=='启用') 不可查看 @else <a class="btn btn-success" onclick="picture_add('密保','/adminvipques/{{$row->id}}')" href="javascript:;">可查看</a> @endif</td>
				<td >{{$row->add_time}}</td>
