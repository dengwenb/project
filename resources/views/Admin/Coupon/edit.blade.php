...
<div class="page-container">
	<form class="form form-horizontal" id="form-article-add" action="/adminCoupon/{{$id}}/edit" method="get">
		...
		<div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="{{$value->min_pri}}" placeholder="" id="" name="min_pri">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>优惠金额</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="{{$value->c_pri}}" placeholder="" id="" name="c_pri">
            </div>
        </div>
        <div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>开始日期：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" value="{{$value->start_time}}" onfocus="WdatePicker({ dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}' })" id="datemin" class="input-text Wdate" name="start_time">
			</div>
		</div>
        <div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>结束日期：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" value="{{$value->stop_time}}" onfocus="WdatePicker({ dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'datemin\')}' })" id="datemax" class="input-text Wdate" name="stop_time">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
                ...
                <option value=""disabled="disabled">请选择</option>
					...
					<option value="0" @if($value->state==0) selected @endif>不显示</option>
					<option value="1" @if($value->state==1) selected @endif>显示</option>
